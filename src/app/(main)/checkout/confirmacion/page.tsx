...
export default async function ConfirmacionPage({ searchParams }: Props) {
  ...
  const TRANSFER_TITULAR = config.titular || "La Parfumerie de Solange";
  void isPending;
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-12">
      ...
      <div className="text-center mb-10">
        ...
        <div className="mt-4 inline-flex items-center gap-2 bg-[#0D0D0D] border border-[#2D2D2D] px-4 py-2 text-[#D4AF37] text-sm font-mono">
      ...
      {pedido.metodo_pago === "efectivo" && (
        <div className="bg-[#0D0D0D] border border-[#1A1A1A] p-6 mb-6">
          <div className="flex items-center gap-3 mb-4">
            <Banknote size={20} className="text-[#D4AF37]" />
            <h2 className="text-white font-semibold">Pago en efectivo</h2>
          </div>
          <p className="text-[#888888] text-sm leading-relaxed">
            El pago se realiza al momento de la entrega o al retirar en tienda en
            <span className="text-white"> Ayala 604, Santa Rosa, La Pampa</span>.
